#!/bin/bash
# Debug status line - logs JSON to file with timestamp
set -euo pipefail
...
printf "\033[36m%s\033[0m%s | \033[35m%s\033[0m | \033[33mctx:%s/%s (%s%%)\033[0m | \033[32m↑%s ↓%s\033[0m | \033[90msid:%s\033[0m" \
    "$DISPLAY_CWD" "$GIT_INFO" "$MODEL" "$TOTAL_K" "$CONTEXT_K" "$USAGE_PCT" "$INPUT_K" "$OUTPUT_K" "${SESSION_ID:0:8}"
